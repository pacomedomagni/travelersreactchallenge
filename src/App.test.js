...
emp1SaveBtn2 = getByTestId(TEST_IDS.emplSaveBtnPrefix + '1')
afterEach(() => {
  cleanup()
})
...
expect (emp1SaveBtn2).toBeDisabled()
...
it('should result in appropriate UI changes when employee salary changes to a new valid value', () => { 
  ...
  emplSalaryDiv2. queryByTestId(TEST_IDS.emplSalaryPiуPrefix + '1')
...
it('adding a new employee with invalid values is not possible', () => { 
  ...
  expect (addNewEmplBtn).toвeEnabled()
